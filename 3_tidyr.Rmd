...
GP <- GP %>%  rename(Participant = Respondent)
joined <- inner_join(GP,IM,by="Participant")
joined <- joined %>% 
  select(-Gender.y,-Condition.y) %>% 
...
```{r exercise 02}
x_id <- c(1:19)
x_value = rnorm(19, mean = 180, sd = 50)
x_df <-as.data.frame(cbind(x_id, x_value))
y_id <- c(1:31)
y_value = rnorm(31, mean = 10, sd = 2)
y_df <-as.data.frame(cbind(y_id, y_value))

crossjoin <- cross_join(x_df, y_df)
innerjoin <- x_df %>%
